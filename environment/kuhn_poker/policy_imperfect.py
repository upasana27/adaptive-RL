    ...
    def __call__(self, obs):
        ...
        if stage == 0:
            ...
            if self_card == 0:
                ...
                # Q, should have been 0
                return np.random.rand() < self.extra_probs[0]
            elif self_card == 2:
                # K, should have been 1
                return np.random.rand() < self.extra_probs[1]
            else:
                raise ValueError("Wrong self card")
        elif stage == 1:
            # faces bet
            if self_card == 0:
                # J, should have been 0
                return np.random.rand() < self.extra_probs[2]
            elif self_card == 1:
                # Q
                ...
                # K, should have been 1
                return np.random.rand() < self.extra_probs[3]
            else:
                raise ValueError("Wrong self card")
        else:
...
def generate_policy_pool(num_policies):
    ...
    np.random.seed(1)
    pool = [KuhnPokerFullPolicy(np.random.rand(6)) for _ in range(num_policies)]
    np.random.set_state(old_state)
    return pool


# test_probs = [[0.25,0.67],[0.75,0.8],[0.67,0.4],[0.5,0.29],[0.28,0.10],[0.17,0.2],[1/3,1/3]]
# test_policies = [KuhnPokerFullPolicy(probs) for probs in test_probs]
...
def get_train_eval_pool(args):
    ...
    print('Generating', args.train_pool_size + args.eval_pool_size, 'potentially dominated policies')
