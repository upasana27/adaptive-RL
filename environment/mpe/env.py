    ...
    def _render(self, mode='human', close=True):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from environment.mpe import rendering
        ...
        if self.render_geoms is None:
            ...
            for entity in self.world.entities:
                ...
                if 'agent' in entity.name:
                    geom.set_color(*entity.color, alpha=entity.alpha)
                    # print('drawing', entity.color, entity.alpha)
...
class MPE(gym.Env):
    def __init__(self, scenario_name, num_agents, num_good_agents, horizon, obs_radius, init_radius,
        ...
        world = scenario.make_multi_world(num_agents, num_good_agents, obs_radius, init_radius,
                                          shaped_reward, collide_reward, collide_reward_once, watch_tower)
        self.env = MultiAgentEnv(world, scenario.reset_world, scenario.reward, scenario.observation,
                                 scenario.info, scenario.done, scenario.post_step, discrete_action=True)
        self.opponent = None
        self.last_obs = None
    ...
    def set_opponent(self, opp: MultiAgentResamplePolicy):
        self.opponent = opp
    ...
    def reset(self):
        ...
        obs = self.env._reset()
        self.last_obs = obs
        # Deeper color for the ego agent
        self.env.world.agents[self.player_id].alpha = 1.0
        self.env.world.agents[self.player_id].color = np.array([1.0, 0.0, 0.0])
        self.steps = 0
        return obs[self.player_id]

    def step(self, action):
        self.steps += 1
        opponent_actions = self.opponent(self.env.world)
        # actions = [None] * len(self.env.world.agents)
        actions = np.zeros((len(self.env.world.agents), 5))
        assert len(self.opponent.current_policies) + 1 == len(self.env.world.agents)
        for pol, act in zip(self.opponent.current_policies, opponent_actions):
            assert pol.id != self.player_id
            actions[pol.id][act] = 1
        actions[self.player_id][action] = 1
        # print(actions)
        obs, rew, done, info = self.env._step(actions)
        info_ = info['n'][self.player_id]
        info_['opponent_act'] = opponent_actions.copy()
        info_['opponent_obs'] = np.concatenate([self.last_obs[pol.id] for pol in self.opponent.current_policies])
        self.last_obs = obs
        return obs[self.player_id], rew[self.player_id], done[self.player_id] or self.steps >= self.horizon, info_

    def seed(self, seed=None):
        self.env._seed(seed)

    def render(self, *args, **kwargs):
        self.env._render(**kwargs)
