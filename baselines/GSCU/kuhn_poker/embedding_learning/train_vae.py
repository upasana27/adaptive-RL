...
def main(version, seed):
    ...
    for epoch_i in range(0, epochs):
        ...
        if is_vae:
            torch.save(encoder.state_dict(), os.path.join(ckp_dir, 'encoder_vae_param_'+version+'_'+str(epoch_i)+'.pt'))
        else:
            torch.save(encoder.state_dict(), os.path.join(ckp_dir, 'encoder_ae_param_'+version+'_'+str(epoch_i)+'.pt'))
        torch.save(decoder.state_dict(),  os.path.join(ckp_dir, 'decoder_param_'+version+'_'+str(epoch_i)+'.pt'))
